...
function normalizeStickyAccessBlock(commandAccess, guildId) {
...
function formatStickyAccessSummary(access) {
  const users = access.sticky.users.length ? access.sticky.users.map((id) => `<@${id}>`).join(', ') : 'none';
  const roles = access.sticky.roles.length ? access.sticky.roles.map((id) => `<@&${id}>`).join(', ') : 'none';
...
async function handleStickyGrantAccess(interaction, commandAccess, saveCommandAccess) {
  ...
  const access = normalizeStickyAccessBlock(commandAccess, interaction.guildId);

  if (!user && !role) {
    await interaction.reply({ content: formatStickyAccessSummary(access), flags: 64 });
    return;
  }
  ...
  if (user) {
    const hasAccess = access.sticky.users.includes(user.id);
    if (revoke) {
  ...
  const hasAccess = access.sticky.roles.includes(role.id);
  if (revoke) {
...
module.exports = {
  data: {
    ...
    options: [
      ...
      {
        name: 'grant',
        description: 'Grant or revoke sticky command access for a user or role',
        type: ApplicationCommandOptionType.SubcommandGroup,
        options: [
          {
            name: 'access',
            description: 'Grant or revoke sticky access, or list current access if no target is provided',
            type: ApplicationCommandOptionType.Subcommand,
            options: [
              {
                name: 'user',
                description: 'User to grant sticky access to',
                type: ApplicationCommandOptionType.User,
                required: false,
              },
              {
                name: 'role',
                description: 'Role to grant sticky access to',
